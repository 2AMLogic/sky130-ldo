...
| 5 | `unmet` / `no_evidence` | The largest real gap, and not a presentation choice. `spec/target-spec.md` *is* ratified (issue #1, DR-006) and a full 45-point PVT campaign exists under `sim/*/`— but it reports **FAIL on 8 of the 12 graded rows** (9 before issue #164 fixed the `mc-output-accuracy` bench's initial-condition contract, which moved `Output` to PASS without touching the DUT or any ratified bound — see `design/README.md` → "#164"), and the records are this repo's own Markdown/JSON format rather than `klt sim` envelopes, so there is nothing here that could render `met` even if the rows passed. Tracked as issues #114–#121. |
...
read as if it did: the report currently reads **FAIL on 8 of the 12 rows that
carry a graded verdict** — Dropout, Line regulation, Load regulation, Load
transient, PSRR, Iq, Thermal and Stability — and PASS on four (Output, Current
