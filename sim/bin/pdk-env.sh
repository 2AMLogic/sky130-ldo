...
#
# Not needed to run the corner runner itself: corner-run.py resolves the PDK
# on its own. This is for driving xschem/ngspice by hand.
...
_pdk_env_out="$(python3 "${_pdk_env_dir}/corner-run.py" --print-env)" || {
  ...
  echo "pdk-env.sh: run 'python3 ${_pdk_env_dir}/corner-run.py --check-env' for a diagnosis" >&2
