...
* FROZEN COPY -- #164 initial-condition screen (variants A and D).
*
* This is `sim/mc-output-accuracy/testbench/tb_mc_output_accuracy.sch` exactly
* as it stood BEFORE #164 changed it: EN is a plain DC level ('vsup'), not the
* PULSE edge the shipped bench now uses. It is frozen here, in its own
* experiment slug, so the screen that diagnosed #164 stays re-runnable from
* committed files after the shipped bench moved on. Do not "fix" it forward --
* its whole evidentiary value is being the configuration under test.
*
* Two of the screen's four variants drive this file, differing only in the
* analysis card their manifest declares (see each `experiment.json`):
*   - variant A (sim/mc-ic-screen-a): `tran 10u 3m`       -- unconstrained `.op` seeds the transient
*   - variant D (sim/mc-ic-screen-d): `tran 10u 3m uic`   -- cold start, every node at 0
* Variant B (sim/mc-ic-screen-b) uses its own copy of this file plus an `.ic`
* card; variant C is the shipped bench itself (sim/mc-output-accuracy).
*
* Both variants carry 14 extra hierarchical `.meas` node-dump cards on top of
* `vout_ss` -- that dump is the measurement, not a side effect: it is what
* shows variant A's twelve out-of-window samples are not circuit states
* (FB above the VOUT node that is the only thing feeding it through a passive
* divider). See design/README.md "#164".
*
