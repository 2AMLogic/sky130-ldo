...
* FROZEN COPY -- #164 initial-condition screen (variant B).
*
* `sim/mc-ic-screen-a/testbench/tb_mc_output_accuracy.sch` (itself the
* pre-#164 shipped bench, EN a DC level) plus ONE added card: an `.ic` that
* seeds the node set at regulation, so the operating-point solve that seeds
* the transient starts from the answer instead of from nothing. `.ic` without
* `uic` constrains the `.op` and is then released for the transient, which is
* exactly the question variant B asks: started AT 1.8 V, does the loop stay
* there, or does it fall into the "rail mode"? See design/README.md "#164".
*
* Variant B differs from variant A ONLY by that card -- the netlist snapshots
* of the two records differ only in the `.ic` line and the `sch_path` comment.
*
...
* ---- variant B's one added card: seed the .op at regulation ----
C {devices/code_shown.sym} -700 -100 0 0 {name=IC_SEED only_toplevel=false value=".ic v(VOUT)=1.8 v(xldo.FB)=1.2 v(xldo.N_FBB)=0.6 v(xldo.EA_OUT)=2.38 v(xldo.EA_CZ)=2.38 v(xldo.SS)=3.3"}
