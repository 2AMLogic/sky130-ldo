...
* #169 re-test of #81 item 2 -- variant V: #81's OWN method, replicated.
*
* #81 item 2 reported a second, stable, non-regulating equilibrium at
* 125 C / 50 mA (tt -> VOUT = 2.093 V, FB = 1.114 V, N_FBB = 1.048 V) from a
* `tran ... uic` whose `.ic` constrained ONLY v(vout) = 1.8 V, with EN a DC
* level, VIN = 3.60 V and an ideal 50 mA sink -- sim/dropout-vs-load's own
* circuit, run as a transient. #81 never committed that deck (it was a local
* probe), so this is its reconstruction from design/README.md "#81 resolved"
* item 2, re-run against the CURRENT design/ldo_3v3in_1v8out.sch (which has
* since been changed by #69 and #116's pass-device re-size). Its job is to
* separate "the circuit changed" from "the method was unreliable": run next
* to variants F/B (the full node set constrained) and C (cold start), on one
* netlist, it shows which of the three initial-condition contracts reaches
* the state #81 recorded.
*
* The ONLY difference from sim/ic-screen-125c-b/testbench/tb_ic_screen_125c_seeded.sch
* is the `.ic` card: here it names v(VOUT) and nothing else. Every other node
* -- FB, N_FBB and EA_CZ (the three res_xhigh_po nodes #164 found to be where
* the unconstrained solve goes wrong), SS, the bias rails, the amplifier's
* internal nodes -- starts at 0 V under `uic`, exactly as in #81.
*
* VIN = 'vsup' (the corner runner's supply; 3.60 V is on the declared axis).
...
S {}
E {}
T {#169 item-2 re-test, variant V -- #81's own method: .ic v(vout)=1.8 ONLY + uic,
