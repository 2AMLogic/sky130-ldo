...
| Load | 0–50 mA (0 mA = no external load; feedback divider is the only inherent preload) | 100 mA | G | Pass-device W = `sky130_fd_pr__pfet_g5v0d10v5`, re-sized by DR-011 (#116) to ≈5 mm-class, superseding DR-003's ≈2.5 mm-class methodology (found sized at the wrong bias point). |
| Dropout @ 50 mA | < 300 mV | < 200 mV | G+S | Test point `V_in = V_out + dropout` (not `V_in_min`), binding corners `{ss, sf}` co-binding at 125 °C — DR-003. Current implementation: FAIL (6/45 PVT corners PASS, up from 0/45; best case 268 mV, closest miss 363 mV), tracked design gap. Pass device re-sized 2.5→5 mm (DR-011/#116); −40/27 °C corners closed at `ss`, meaningfully narrowed elsewhere, but 125 °C corners remain dominated by a pre-existing dc-sweep solver-fragility (mechanism 4, #81/#79) the resize made more pervasive, not introduced. |
...
| Load transient | 1↔50 mA step, ~1 µs edges: peak excursion ≤ 150 mV, recover to ±1% in ≤ 20 µs, over the ratified C_out/ESR window | peak ≤ 100 mV | G | C_out/ESR window: 0.33–4.7 µF, 0–500 mΩ, no minimum ESR, ceramic-stable — DR-002. Current implementation (per `sim/` record `20260926-013955-6449a98`, the first full 45-corner run of the post-#119 manifest — both clauses measured, at DR-002's 4.7 µF window ceiling — superseding `20260825-081255-4cb27f8`): FAIL (0/45 PVT corners), tracked design gap. The row's two clauses now split cleanly. The peak-excursion clause passes 45/45 at both sub-metrics against the ratified ≤ 150 mV — `undershoot_v` 0.063–0.119 V, `overshoot_v` 0.049–0.092 V — confirming #119's `C_out`-sizing fix across the whole matrix (it was 25/45 on the superseded record). The recovery-time clause (`recovery_rise_us`/`recovery_fall_us`) fails 45/45 against the ratified ≤ 20 µs — 74.7–132.1 µs rise, 425.7–796.1 µs fall — for reasons `C_out` sizing cannot address (see `design/README.md`'s "#119" section and DR-002's append), so every one of the 45 corner FAILs is the recovery clause alone. |
...
| Thermal | continuous worst-case dissipation set by Vin_max × I_load; short-circuit ceiling set by the current limit; specified to Tj ≤ 125 °C (rated-operation ceiling, unchanged); θJA delegated to package/integration | — | G | Fault-only thermal-shutdown backstop above this ceiling: 150 °C nominal trip / 135 °C nominal reset, auto-restart, internally-generated bias-derived reference (not `VREF`, not a bandgap) — DR-005. 150 °C sits outside this repo's characterized 125 °C temperature ceiling and is PVT-loose. Current implementation: FAIL (12/15 corners) — all three failing corners fail only the backstop's auto-restart hysteresis-sign bound (`hysteresis_c`), not a θJA bound; tracked design gap — see DR-005 Consequences and `measurements/characterization.md`. Delegated θJA made explicit as an integration constraint, θJA ≤ (125 °C − Ta)/93.4 mW for rated operation — [`DR-008`](decision-records/DR-008-thermal-theta-ja-integration-constraint.md) (`proposed`). |
