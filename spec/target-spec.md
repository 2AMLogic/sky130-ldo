...
| Load transient | 1↔50 mA step, ~1 µs edges: peak excursion ≤ 150 mV, recover to ±1% in ≤ 20 µs, over the ratified C_out/ESR window | peak ≤ 100 mV | G | C_out/ESR window: 0.33–4.7 µF, 0–500 mΩ, no minimum ESR, ceramic-stable — DR-002. Current implementation (per `sim/` record `20260825-081255-4cb27f8`, which measured only the peak-excursion clause): FAIL (25/45 PVT corners), tracked design gap. #119 (2026-09-25) instrumented the row's recovery-time clause for the first time and screened 21/45 corners against a `C_out`-sizing fix: peak excursion closes (8/8 re-verified FAILs pass), but `recovery_rise_us`/`recovery_fall_us` fail at every screened corner for reasons `C_out` sizing cannot address (see `design/README.md`'s "#119" section and DR-002's append) — a full 45-corner superseding record is pending (host-instability-blocked, tracked by #166). |
