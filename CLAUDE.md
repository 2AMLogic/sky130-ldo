# sky130-ldo — agent instructions

Private canary block (for now): a low-dropout regulator on the sky130 open PDK,
...
- **Port parity**: the spec and structure mirror the ratified
  `2AMLogic/gf180-ldo` — same block, two PDKs. Prefer aligning with it; where
...
- **Harness bootstrap**: copy the sim-harness and `klt` layout-flow patterns from
  `2AMLogic/sky130-bandgap` (sky130 flow) and `2AMLogic/gf180-ldo` (LDO
